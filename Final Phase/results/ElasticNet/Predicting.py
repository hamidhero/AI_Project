...
for item in directors.keys():
	...
	# print parameter_1
	# print parameter_2
	clf = linear_model.ElasticNet(alpha = .9)
...
for item in writers.keys():
	number_of_movies = len(writers[item])
	parameter_1 = []
	for i in range(number_of_movies):
		parameter_1.append([i+1])

	parameter_2 = []
	sorted_by_year_list = sorted(writers[item], key=itemgetter(2))
	for movie in sorted_by_year_list:
		parameter_2.append(float(sorted_by_year_list[i][1]))
	...
	clf.fit(parameter_1, parameter_2)
	predicted_next_movie_rate = clf.predict([number_of_movies + 1])
	predicted_writers[item] = predicted_next_movie_rate
